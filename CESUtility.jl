...
function ces_node(
    ...
    for i in keys(quantities)
        push!(nests, ch_pr[i]) # pulling from the Dict I created that has children as keys and direct parents as values
    end
...
function CESutility_multinat(bnchdata::NamedMatrix)
    # Leaves [:ugs,:coa] into :homefuels
    homefuel_i,homefuel_ute = 
        ces_node(Dict{Symbol, Float64}(:ugs => bnchdata[:ugs,:pce],:coa => bnchdata[:coa,:pce]), 
        Dict{Symbol, Float64}(:coa=>value(compensated_demand(FDem,PA[:coa])),:ugs=>value(compensated_demand(FDem,PA[:ugs]))))
    ...
    # bnch_quants = merge(fuels_i, veh_exp_i)
    # quantities = merge(fuels_ute, veh_exp_ute)
    ...
    # bnch_quants = merge(non_pers_transp_i, pers_transp_i)
    # quantities = merge(non_pers_transp_ute, pers_transp_ute)

    # Nest transp, non_transp=>s,
    s_i, s_ute = ces_node(merge(transp_i, non_transp_i),merge(transp_ute, non_transp_ute))
    # bnch_quants = merge(transp_i, non_transp_i)
    # quantities = merge(transp_ute, non_transp_ute)

        return only(values(s_ute))
    end

    # # Edited from elastiticity vector
    # ### Keys are the ends, acomasdfasdfasdfnd values are the nodes above
