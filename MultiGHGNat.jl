...
## CH4 Emissions intensity of Standard value-added activities: total emissions/total value-added cost: MMt/BillUS$
    VASInt = DenseAxisArray(fill(0.,length(J)),J)
    for c in CH4sectors
        VASInt[c] =  MAC_CH4_WiNDC_tot[1,c]/MAC_CH4_WiNDC_tot[2,c]
    end
...
for c in CH4sectors
    ...
    # @aux_constraint(MultiNat, CH4em[c],  CH4em[c] - (VAS[c]*MAC_CH4_WiNDC_tot[1,c]+VAM[c]*MAC_CH4_WiNDC_tot[1,c]*ch4VAMInt[c]/ch4VASInt[c]))
