...
# function timeWiNnat(n::Int64)
	...
	fd = S[:fd] # "BEA Final demand categories",
	...
	year = Symbol(2017)
	# PARAMETERS
	# TODO: It's not cleat that this is filling the values appropriately
	...
	# PARAMETERS For a single year, knock out one dimension
	...
	# The best I can tell is that at some point of data the i and j might be different, but in the csvs I have they're exactly the same
	# Which also seems like it must be true re the GAMS line alias(i, j);...
	...
	# 	# [		[Input(PA[i], P[:id_0][year,i,j]) for i in sectorsi];                        
	#     # 	[Input(
	# 	# 				Nest(:VA, 1.,
	...
	for m in margin
		...
		[Output(PM[m], sum(ms_0[i,m] for i in sectorsi) ) for m in margin],# in sectorsi],  # *&* "for i in sectors i" for the sum?
	...
	add!(WiNnat, DemandFunction(RA, 1.,
		...
		#Endowment(PA[i], -sum(P[:fd_0][year,i,:])+ P[:fd_0][year,i,:pce] for i in sectorsi), 

		   #xfd seems like it's fd_0[i], but without the fd,fd.
		 [Endowment(PVA[va], sum(va_0[va,:])) for va in valueadded]
		 ;
		 Endowment(PFX, bopdef_0)
		]))
	# LAST THING (for first pass), What syntax to sum of j (meaning pick a j, sum all of va for that j, next j, sum all of va for that j etc. ?)
